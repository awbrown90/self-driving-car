...
class Environment(object):
    ...
    def __init__(self, num_dummies=30): # num_dummies was 3
    ...
    def step(self):
        ...
        self.primary_agent.Move() #move primary agent to best move based its current state from current environment as seen in the GUI
        ...
        for agent in self.agent_states.iterkeys():
            if agent is self.primary_agent:
                continue #we should have already updated primary agent first.
            else:
                agent.update(self.t)
                
        self.primary_agent.update(self.t) #Now that all the other agents have been updated, re-update primary agent's inputs and learning to reflect new GUI scene

        if self.done:
            self.avg_violations += self.violations
        ...
        if self.primary_agent is not None:
            ...
            if agent_deadline <= self.hard_time_limit:
                self.done = True
                print "Environment.step(): Primary agent hit hard time limit ({})! Trial aborted with {} violations.".format(self.hard_time_limit,self.violations)
            elif self.enforce_deadline and agent_deadline <= 0:
                self.done = True
                self.avg_violations += self.violations
                print "Environment.step(): Primary agent ran out of time! Trial aborted with {} violations.".format(self.violations)
    ...
    def act(self, agent, action):
        ...
        if action == 'forward':
            ...
            if not (light == 'green' and (inputs['oncoming'] == None or inputs['oncoming'] == 'left')):
                move_okay = False
        elif action == 'right':
            heading = (-heading[1], heading[0])
            if not (light == 'green' or inputs['left'] != 'forward'):
                move_okay = False
        ...
        if move_okay: # very small negative reward to motivate quickly arriving to the destination
            reward = -0.04
        else:
            # was there a traffic voliation
            reward = -30
            if agent is self.primary_agent:
                self.violations += 1
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                ...
                self.done = True
                self.goal_reaches += 1
                print "Environment.act(): Primary agent has reached destination with {} violations".format(self.violations) 
    ...
    def update_text(self, agent, action, reward, deadline):
        self.status_text = "state: {}\naction: {}, last reward: {}, trial: {}, deadline: {}, violations: {}".format(agent.get_state(), action, reward, self.trial, deadline, self.violations)

    def score(self):
        print "out of {} trials, the agent had {}% success rate of getting to the destination within the time limit, and on avg had {} violations per trial".format(self.n_trials,self.goal_reaches/float(self.n_trials)*100,self.avg_violations/float(self.n_trials))
